...
async function fetchRepairCost(vehicle_id: string){
    const res = await fetch(`/api/vehicles/${vehicle_id}/analytics/repair-cost-overtime`);
    if (!res.ok) {
...
export function RepairCostOvertime({vehicle_id}:
  {vehicle_id: string}
) {
        ...
        useQuery({
            queryKey: [`vehicle-${vehicle_id}-analytics-repair-cost-overtime`],
            queryFn: () => fetchRepairCost(vehicle_id),
            staleTime: 1000 * 30,
        },);
